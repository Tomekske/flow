...
class StatsHelper {
  ...
  static Map<String, String> calculateStats(List<Log> logs, {DateTime? now}) {
    ...
    final double avgInterval = calculateAverageInterval(sortedLogs, DateTime.now());
  ...
  static List<Log> sortLogs(List<Log> logs) {
    return List<Log>.from(logs)
      ..sort((a, b) => a.timestamp.compareTo(b.timestamp));
  }

  /// Calculates visits per hour based on the specific time window
  static double calculateHourlyRate(List<Log> sortedLogs, DateTime? now) {
    DateTime startTime;
    DateTime endTime;
    ...
    if (now == null && hoursDiff < 1) hoursDiff = 1; // Minimum 1 hour window for global stats
  ...
  static double calculateAverageInterval(List<Log> sortedLogs, DateTime now) {
    ...
    for(var log in sortedLogs) {
      var interval = getInterval(log.timestamp);

      intervalMap[interval]?.add(log.timestamp);
    }
    ...
    double sum = x.values.fold(0, (previousValue, element) => previousValue + element).toDouble();
